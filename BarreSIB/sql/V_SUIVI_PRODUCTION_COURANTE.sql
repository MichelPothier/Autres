DROP VIEW V_SUIVI_PRODUCTION_COURANTE;

/* Formatted on 2014-08-18 15:33:24 (QP5 v5.240.12305.39446) */
CREATE OR REPLACE FORCE VIEW V_SUIVI_PRODUCTION_COURANTE
(
   NO_MAP,
   NO_LOT,
   TY_PRODUIT,
...
AS
   SELECT DISTINCT CAST (F502_PS.NO_MAP AS NUMBER (10)),
          ...
          CAST(NVL(F603_IT.NO_SEQ,0) AS NUMBER (5)) AS NO_SEQ_ITER,
          F602_ET.CD_OPER,
          F005_US.PRENOM,
          F005_US.NOM,
          F606_GE.ITERATIVE,
    ...
    WHERE     F502_PS.E_PLSNRC = 'P'
          AND F602_ET.NO_MAP = F603_IT.NO_MAP(+)
...
COMMENT ON TABLE V_SUIVI_PRODUCTION_COURANTE IS 'COMMENTAIRE:
Vue permettant de montrer le suivi de production complet.';

CREATE OR REPLACE PUBLIC SYNONYM V_SUIVI_PRODUCTION FOR V_SUIVI_PRODUCTION_COURANTE;

GRANT SELECT ON V_SUIVI_PRODUCTION_COURANTE TO MDDTN;

GRANT SELECT ON V_SUIVI_PRODUCTION_COURANTE TO SIBLIRE;

GRANT SELECT ON V_SUIVI_PRODUCTION_COURANTE TO SIBLIREDIFF;

GRANT SELECT ON V_SUIVI_PRODUCTION_COURANTE TO SIBMAJ;
