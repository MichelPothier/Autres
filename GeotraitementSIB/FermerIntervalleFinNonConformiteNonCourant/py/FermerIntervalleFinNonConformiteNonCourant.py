...
#####################################################################################################################################
# Nom       : FermerIntervalleFinNonConformiteNonCourant.py
# Auteur    : Michel Pothier
# Date      : 01 avril 2015

"""
Outil qui permet de fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.

-L'intervalle d’'édition-version est fermée selon le numéro d'édition et version maximale des identifiants non-conrant. 
...
Attention : Seule les non-conformités non-courant peuvent être traitées par ce programme.
...
Usage:
    FermerIntervalleFinNonConformiteNonCourant.py env produit no_nc identifiant

Exemple:
    FermerIntervalleFinNonConformiteNonCourant.py SIB_PRO RHN 02997 021M07:1:0:BDG;021M08:2:1:BDG

"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: FermerIntervalleFinNonConformiteNonCourant.py 2057 2016-06-15 21:03:15Z mpothier $"
...
#*******************************************************************************************
class FermerIntervalleFinNonConformiteNonCourant(object):
#*******************************************************************************************
    """
    Permet de fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement pour fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement pour fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.
        ...
        for item in identifiant.split(","):
            ...
            if resultat:
                ...
                arcpy.AddMessage(sql)
                raise Exception("L'dentifiant ne doit pas posséder un jeu courant.")
            
            #Construire la sql pour vérifier si l'identifiant possède un jeu courant
            sql = ("SELECT ED_FIN, VER_FIN"
                   "  FROM F705_PR"
                   " WHERE TY_PRODUIT='" + produit + "' AND IDENTIFIANT='" + id + "' AND ED_DEBUT=" + ed_debut + " AND VER_DEBUT=" + ver_debut)
            #Exécuter la requête
            resultat = self.Sib.requeteSib(sql)
            ...
            if ed_fin == 99999:
                ...
                #Vérifier la présence du paramètre
                if (len(resultat) == 0):
                    raise Exception("Aucun identifiant n'est présent dans la vue V200_IN")
                ...
                if float(str(ed_debut) + "." + str(ver_debut)) > float(str(ed_max) + "." + str(ver_max)):
                    #Retourner une erreur
                    raise Exception("Ed.Ver_Début:" + str(float(str(ed_debut) + "." + str(ver_debut))) + " > Ed.Ver_Cour:" + str(float(str(ed_max) + "." + str(ver_max))))
                #Afficher les valeurs
                #arcpy.AddMessage(str(resultat))
        ...
        #S'il reste des identifiants non fermés
            ...
            arcpy.AddWarning("Les identifiants suivants n'ont pas d'intervalle de fin : " + str(resultat))
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet pour fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.
        oFermerIntervalleFinNonConformiteNonCourant = FermerIntervalleFinNonConformiteNonCourant()
        
        #Valider les paramètres obligatoires
        oFermerIntervalleFinNonConformiteNonCourant.validerParamObligatoire(env, produit, no_nc, identifiant)
        
        #Exécuter le traitement pour fermer les intervalles de fin d'une non-conformité pour des identifiants sans jeu courant.
        oFermerIntervalleFinNonConformiteNonCourant.executer(env, produit, no_nc, identifiant, corriger)
