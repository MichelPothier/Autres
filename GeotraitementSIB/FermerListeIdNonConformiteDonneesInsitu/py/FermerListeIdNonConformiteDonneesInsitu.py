...
#####################################################################################################################################
# Nom       : FermerListeIdNonConformiteDonneesInsitu.py
# Auteur    : Michel Pothier
# Date      : 19 septembre 2016

"""
Outil qui permet de fermer une liste d'identifiants d'une non-conformité sur les données géographique après avoir
...
identifiants    : Liste des identifiants de la non-conformité à fermer.
...
-------
 Nom                            Description
 -----------------------------  --------------------------------------------------------------------
 FermerListeIdNonConformiteDonneesInsitu     Fermer une non-conformité sur les données géographique.
"""
#********************************************************************************************
__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: FermerListeIdNonConformiteDonneesInsitu.py 2130 2016-09-20 12:57:30Z mpothier $"
#********************************************************************************************
...
#*******************************************************************************************
class FermerListeIdNonConformiteDonneesInsitu(object):
#*******************************************************************************************
    ...
    #-------------------------------------------------------------------------------------
    def executer(self, env, no_nc, identifiants, no_tache, procdesc):
    #-------------------------------------------------------------------------------------
        ...
        if resultat[0][1] <> None:
            raise Exception("La non-conformité a déjà été traitée : date_traitement=" + str(resultat[0][1]))
        ...
        sql = "SELECT TY_PRODUIT, IDENTIFIANT, ED_DEBUT, VER_DEBUT, ED_FIN, VER_FIN FROM F705_PR WHERE NO_NC='" + no_nc + "' AND IDENTIFIANT IN ('" + identifiants.replace(",","','") + "') ORDER BY IDENTIFIANT"
        arcpy.AddMessage(sql)
        #Exécuter la requête
        resultat = self.Sib.requeteSib(sql)
        #Vérifier la présence du paramètre
...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) > 3:
            identifiants = sys.argv[3].replace(";",",")
        
        if len(sys.argv) > 4:
            no_tache = sys.argv[4].upper()
        
        if len(sys.argv) > 5:
            procdesc = sys.argv[5].upper()
        
        # Définir l'objet de fermeture d'une non-conformité de correction des données géographique
        oFermerListeIdNonConformiteDonneesInsitu = FermerListeIdNonConformiteDonneesInsitu()
        
        # Exécuter le traitement de fermeture d'une non-conformité de correction des données géographique
        oFermerListeIdNonConformiteDonneesInsitu.executer(env, no_nc, identifiants, no_tache, procdesc)
