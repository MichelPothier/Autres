...
#####################################################################################################################################
# Nom : AjouterSelectIdentifiantNonConformite.py
# Auteur    : Michel Pothier
# Date      : 13 septembre 2016

"""
    Application qui permet d'ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
    ...
    Usage:
        AjouterSelectIdentifiantNonConformite.py env no_nc ty_produit decoupage attribut 

    Exemple:
        AjouterSelectIdentifiantNonConformite.py SIB_PRO 03034 BDG GES_DECOUPAGE_50K_2 DATASET_NAME

"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: AjouterSelectIdentifiantNonConformite.py 2115 2016-09-15 16:50:51Z mpothier $"
...
#*******************************************************************************************
class AjouterSelectIdentifiantNonConformite(object):
#*******************************************************************************************
    """
    Permet d'ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
    """
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement pour ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
    ...
    #-------------------------------------------------------------------------------------
    def validerParamObligatoire(self, env, no_nc, ty_produit, decoupage, attribut):
    #-------------------------------------------------------------------------------------
        """
        ...
        if (len(decoupage) == 0):
            raise Exception(u"Paramètre obligatoire manquant: %s" %'decoupage')
        
        if (len(attribut) == 0):
            raise Exception(u"Paramètre obligatoire manquant: %s" %'attribut')
        
        #Sortir
        return
   
    #-------------------------------------------------------------------------------------
    def executer(self, env, no_nc, ty_produit, decoupage, attribut):
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement pour ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
        ...
        idList = list(set("{0}".format(row[0]) for row in arcpy.da.SearchCursor(decoupage, [attribut])))
        idList.sort()
        #Traiter tous les éléments sélectionnés dans la classe de découpage
        for identifiant in idList:
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet pour ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
        oAjouterSelectIdentifiantNonConformite = AjouterSelectIdentifiantNonConformite()
        
        #Valider les paramètres obligatoires
        oAjouterSelectIdentifiantNonConformite.validerParamObligatoire(env, no_nc, ty_produit, decoupage, attribut)
        
        #Exécuter le traitement pour ajouter des identifiants d'un produit pour une non-conformité selon une sélection d'éléments contenue dans un Layer de découpage.
        oAjouterSelectIdentifiantNonConformite.executer(env, no_nc, ty_produit, decoupage, attribut)   
