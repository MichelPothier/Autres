...
#####################################################################################################################################
# Nom : CreerGroupeProduction.py
# Auteur    : Michel Pothier
# Date      : 05 février 2015

"""
    Application qui permet de créer un ou plusieurs groupes d'un lot de production dans SIB.
    ...
    noGroupe           OB     Numéro de groupe d'un lot de production. Un groupe constitu un sous ensemble du lot. 
                              ...
                              défaut = <numéro de groupe maximum du lot + 1>
                              ATTENTION : Tous les numéros de groupe supérieur au maximum des groupes du lot seront créés.
                              Exemple : Si le groupe maximum du lot est 4 et que le groupe spécifié est 8, alors les groupes 5,6,7 et 8
                              seront créés avec les mêmes valeurs des autres paramètres (dateRecep,precDeficiente,boniPenalite).
                              La valeur maximum d'un groupe est de 99.
    ...
    Usage:
        CreerGroupeProduction.py env noLot noGroupe [dateReception] [qualContenu] [precDeficiente] [boniPenalite]

    Exemple:
        CreerGroupeProduction.py SIB_PRO 35444 5 2015-02-03 0 0 0

"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: CreerGroupeProduction.py 2057 2016-06-15 21:03:15Z mpothier $"
...
#*******************************************************************************************
class CreerGroupeProduction(object):
#*******************************************************************************************
    """
    Permet de créer un ou plusieurs groupes d'un lot de production dans SIB.
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement pour créer un ou plusieurs groupes d'un lot de production dans SIB.
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement pour créer un ou plusieurs groupes d'un lot de production dans SIB.
        ...
        #Valider le groupe du lot de production
        arcpy.AddMessage("- Valider le groupe du lot de production")
        resultat = oSib.requeteSib("SELECT MAX(GROUPE) FROM F601_GR WHERE NO_LOT='" + noLot + "'")
        #Vérifier le résultat
        if resultat:
            #Vérifier si le groupe du lot de production est valide
            maxGroupe = int(resultat[0][0])
            if int(noGroupe) <= maxGroupe:
                #Retourner une exception
                raise Exception("Le groupe du lot de production est inférieur au maximum des groupes : " + noGroupe + "<=" + str(maxGroupe))
        ...
        #Creer l'information du groupe du lot de production
        arcpy.AddMessage("- Creer l'information du groupe du lot de production")
        #Traiter tous les groupes supérieur au maximum des groupes du lot
        for groupe in range(maxGroupe+1,int(noGroupe)+1):
            #Initialiser la commande SQL de modifification
            sql = "INSERT INTO F601_GR VALUES (P0G03_UTL.PU_HORODATEUR,'" + sUsagerSib + "',SYSDATE,SYSDATE,'" + noLot + "','" + str(groupe).rjust(2,'0') + "'"
            sql = sql + ",TO_DATE(" + dateReception + ")," + qualContenu + "," + precDeficiente + "," + boniPenalite + ")"
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet pour créer un ou plusieurs groupes d'un lot de production dans SIB.
        oCreerGroupeProduction = CreerGroupeProduction()
        
        #Valider les paramètres obligatoires
        oCreerGroupeProduction.validerParamObligatoire(env, noLot, noGroupe)
        
        #Exécuter le traitement pour créer un ou plusieurs groupes d'un lot de production dans SIB.
        oCreerGroupeProduction.executer(env, noLot, noGroupe, dateReception, qualContenu, precDeficiente, boniPenalite)
