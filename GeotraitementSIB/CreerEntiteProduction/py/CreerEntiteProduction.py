...
####################################################################################################################################
# Nom       : CreerEntiteProduction.py
# Auteur    : Michel Pothier
# Date      : 03 août 2016

"""
    Application qui permet de créer un nouveau code d'élément topographique (entité de production) pour toutes les normes de la BDG.
    ...
    Usage:
        CreerEntiteProduction.py env codeElem descFrancais descAnglais

    Exemple:
        CreerEntiteProduction.py SIB_PRO "1000000" "Aire désignée point" "Designated area point"
"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: CreerEntiteProduction.py 2092 2016-08-17 13:55:49Z mpothier $"
...
#*******************************************************************************************
class CreerEntiteProduction:
#*******************************************************************************************
    """
    Classe qui permet de créer un nouveau code d'élément topographique
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement pour créer un nouveau code d'élément topographique
        (entité de production) pour toutes les normes de la BDG.
        ...
        sql = "SELECT CD_ELEM_TOPO, DESCR, DESCR_AN FROM F132_EB WHERE CD_ELEM_TOPO='" + codeElem + "'"
        arcpy.AddMessage(sql)
        resultat = self.Sib.requeteSib(sql)
        #Vérifier le résultat
        if resultat:
            #Retourner une exception
            raise Exception(u"Le code d'élément topographique est déjà présent : " + codeElem + " : " + resultat[0][1] + " : " + resultat[0][2])
        
        #---------------------------------------------------------------------
        #Créer le nouveau code d'élément topographique
        arcpy.AddMessage(" ")
        arcpy.AddMessage("- Créer le nouveau code d'élément topographique ...")
        ...
        sql = "INSERT INTO F132_EB VALUES (P0G03_UTL.PU_HORODATEUR, '" + sUsagerSib + "', SYSDATE, SYSDATE, '" + codeElem + "', '" + descFrancais.replace("'","''") + "', '" + descAnglais.replace("'","''") + "')"
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet pour créer un nouveau code d'élément topographique (entité de production) pour toutes les normes de la BDG.
        oCreerEntiteProduction = CreerEntiteProduction()
        
        #Valider les paramètres obligatoires
        oCreerEntiteProduction.validerParamObligatoire(env, codeElem, descFrancais, descAnglais)
        
        #Exécuter le traitement pour créer un nouveau code d'élément topographique (entité de production) pour toutes les normes de la BDG.
        oCreerEntiteProduction.executer(env, codeElem, descFrancais, descAnglais)
