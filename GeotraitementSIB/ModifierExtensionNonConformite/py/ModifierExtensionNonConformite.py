...
#####################################################################################################################################
# Nom : ModifierExtensionNonConformite.py
# Auteur    : Michel Pothier
# Date      : 6 janvier 2015

"""
    Application qui permet de modifier une extension pour un numéro de non-conformité dans SIB.
    ...
    Usage:
        ModifierExtensionNonConformite.py env no_nc date_ech resp_ech note

    Exemple:
        ModifierExtensionNonConformite.py SIB_PRO 03645 2015-01-16 MPOTHIER 'Note explication de l'extension'

"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: ModifierExtensionNonConformite.py 2057 2016-06-15 21:03:15Z mpothier $"
...
#*******************************************************************************************
class ModifierExtensionNonConformite(object):
#*******************************************************************************************
    """
    Permet de modifier une extension pour un numéro de non-conformité dans SIB.
    """
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement de modification d'une extension pour un numéro de non-conformité dans SIB.
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement de modification d'une extension pour un numéro de non-conformité dans SIB.
        ...
        sql = "SELECT DATE_ECH FROM F703_EX WHERE NO_ENREG='" + no_nc + "' AND TYPE_ISO='NC' AND DATE_ECH=TO_DATE('" + date_ech + "','YYYY-MM-DD')"
        arcpy.AddMessage(sql)
        resultat = self.Sib.requeteSib(sql)
        #Vérifier le résultat
        if len(resultat) == 0:
            #Retourner une exception
            raise Exception("La date d'échéance est absente")
        ...
        arcpy.AddMessage("- Modifier une extension pour le numéro de non-conformité")
        sql = "UPDATE F703_EX SET ETAMPE='" + sUsagerSib + "',DT_M=SYSDATE,RESP_ECH='" + resp_ech + "',NOTE='" + note.replace("'", "''") + "',UPDT_FLD=P0G03_UTL.PU_HORODATEUR WHERE NO_ENREG='" + no_nc + "' AND TYPE_ISO='NC' AND DATE_ECH=TO_DATE('" + date_ech + "','YYYY-MM-DD')"
        arcpy.AddMessage(sql)
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet de modification d'une extension pour un numéro de non-conformité dans SIB.
        oModifierExtensionNonConformite = ModifierExtensionNonConformite()
        
        #Valider les paramètres obligatoires
        oModifierExtensionNonConformite.validerParamObligatoire(env,no_nc,date_ech,resp_ech,note)
        
        #Exécuter le traitement de modification d'une extension pour un numéro de non-conformité dans SIB.
        oModifierExtensionNonConformite.executer(env,no_nc,date_ech,resp_ech,note)   
