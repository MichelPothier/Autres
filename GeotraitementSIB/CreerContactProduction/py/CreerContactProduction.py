...
#####################################################################################################################################
# Nom : CreerContactProduction.py
# Auteur    : Michel Pothier
# Date      : 13 janvier 2015

"""
    Application qui permet de créer un nouveau contact pour un exécutant de production dans SIB.
    ...
    Usage:
        CreerContactProduction.py env cd_execu no_contact nom langue sexe adr_email tel poste fax

    Exemple:
        CreerContactProduction.py SIB_PRO CIT 1386 "Michel Pothier" F M:Masculin mpothier@rncan.gc.ca 819-564-5600 281 819-823-6669

"""

__version__ = "--VERSION-- : 1"
__revision__ = "--REVISION-- : $Id: CreerContactProduction.py 2057 2016-06-15 21:03:15Z mpothier $"
...
#*******************************************************************************************
class CreerContactProduction(object):
#*******************************************************************************************
    """
    Permet de créer un nouveau contact pour un exécutant de production dans SIB.
    """
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement de création d'un nouveau contact pour un exécutant de production dans SIB.
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement de création d'un nouveau contact pour un exécutant de production dans SIB.
        ...
        #Valider si numéro de contact de production est déjà présent
        arcpy.AddMessage("- Valider le numéro de contact de production")
        resultat = self.Sib.requeteSib("SELECT NO_CONTACT FROM F607_CO WHERE NO_CONTACT=" + no_contact)
        #Vérifier si l'exécutant de production est présent
        if len(resultat) > 0:
            #Retourner une exception
            raise Exception("Le numéro de contact de production '" + no_contact + "' est déjà présent")
        ...
        #Créer le contact de production
        arcpy.AddMessage("- Créer le contact de production")
        sql = "INSERT INTO F607_CO VALUES (P0G03_UTL.PU_HORODATEUR,'" + sUsagerSib + "',SYSDATE,SYSDATE,'" + cd_execu + "'," + no_contact + ",'" + nom + "','" + langue + "','" + sexe + "','" + adr_email  + "','"  + tel + "'," + poste + "," + fax + ")"
...
if __name__ == '__main__':
    ...
    try:
        ...
        #Définir l'objet de création d'un nouveau contact pour un exécutant de production dans SIB.
        oCreerContactProduction = CreerContactProduction()
        
        #Valider les paramètres obligatoires
        oCreerContactProduction.validerParamObligatoire(env,cd_execu,no_contact,nom,langue,sexe,adr_email,tel)
        
        #Exécuter le traitement de création d'un nouveau contact pour un exécutant de production dans SIB.
        oCreerContactProduction.executer(env,cd_execu,no_contact,nom,langue,sexe,adr_email,tel,poste,fax)
