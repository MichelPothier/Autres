...
#********************************************************************************************
"""
Outil qui permet d'arrêter le traitement du WatchDog qui repart le serveur XmlRPC à toutes les 5 minutes s'il est arrêté.
...
-------
 Nom                            Description
 -----------------------------  --------------------------------------------------------------------
 ArreterWatchDog                Arrêter le traitement du WatchDog.
"""
#********************************************************************************************
...
#*******************************************************************************************
    """
    Arrêter le traitement du WatchDog.
    ...
    #-------------------------------------------------------------------------------------
        """
        Initialisation du traitement pour arrêter le traitement du WatchDog.
    ...
    #-------------------------------------------------------------------------------------
        """
        Exécuter le traitement pour arrêter le traitement du WatchDog.
        ...
        #Vérifier si le fichier du WatchDog est déjà arrèté
        if os.path.exists(repertoire + fichierOff):
            #Renommer le fichier du WatchDog
            arcpy.AddMessage(" ")
            arcpy.AddWarning("- Le fichier du WatchDog est déjà en mode désactif !")
            arcpy.AddWarning(repertoire + fichierOff)
            
        #Vérifier si le fichier du WatchDog actif est présent
        elif os.path.exists(repertoire + fichierOn):
            #Renommer le fichier du Watchdog pour le mettre en mode désactif
            arcpy.AddMessage(" ")
            arcpy.AddMessage("- Renommer le fichier du WatchDog pour le mettre en mode désactif !")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Définir l'objet pour arrêter le traitement du WatchDog.
